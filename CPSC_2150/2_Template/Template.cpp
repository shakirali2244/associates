...
Template::Node* Template::cons(char x, Node* p){
   return new Node{x, p};
}

bool Template::append(char a){
	return append(a,list);
}

bool Template::append(char a, Node* head){
	if (length() == 0){
		list = cons(a, nullptr);
		return true;
	}
...
bool Template::remove(char a, Node* head, Node* previous){
	if( head == nullptr ){
		return false;
	}
...
bool Template::search(char a){
	return search(a, list);
}

bool Template::search(char a, Node* head){
	if(head == nullptr){
		return false;
	}else if(head->info == a){
		return true;
...
char Template::getFirst(){
	if(list != nullptr) return list->info;
	else return '\0';
}
	

void Template::printList(std::ostream& fout) {
   printList(fout, list);
}

void Template::reverse(){
	if (length() <= 1) return;
	if (length() == 2){
		Node* tmp = list;
		list = list->next;
...
void Template::printList(std::ostream& fout, Node* p) {
   fout << "[ ";
   while (p != nullptr){
      fout << p -> info;
      if (p->next != nullptr)
